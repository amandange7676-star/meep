...
// Create two versions of header HTML
const headerSectionForIndex = headerSection.clone();
const headerSectionForOthers = headerSection.clone();

headerSectionForIndex.find(`.${globalHeader}-pagename`).remove().end().find(`.${globalHeader}-section`).css('display','inline-block');


// Remove .header-section-area only from the version used for other pages
 headerSectionForOthers.find(`.${globalHeader}-section`).remove().end().find(`.${globalHeader}-pagename`).css('display','inline-block');


// Store HTML strings
const headerHTMLForIndex = headerSectionForIndex.html();
const headerHTMLForOthers = headerSectionForOthers.html();
...
let footerHTML = footerSection.html();
...
<div id="header">
  ${
    (fileName.toLowerCase() === "index" || fileName.toLowerCase() === "home")
      ? headerHTMLForIndex
      : (() => {
          const $tmp = $('<div>').append(headerHTMLForOthers);
          $tmp.find(`.${globalHeader}-pagename .Page_name`)
...
</div>   <div id="mainPageContent">${pageContent}</div>
    <div id="footer">${footerHTML}</div>
            </div>
            <script src="assets/js/jquery.js"></script>
            <script src="assets/js/init.js"></script>
