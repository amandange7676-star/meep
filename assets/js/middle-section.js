...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(".ourteam-carousel1").each(function () {
    $(this).owlCarousel({
      loop: true,
      margin: 20,
      nav: true,
      navText: ["<span class='nav-btn prev-btn'>&#10094;</span>", "<span class='nav-btn next-btn'>&#10095;</span>"],
      center: true,   // keeps the middle item highlighted
      autoplay: true,
      autoplayTimeout: 2000, // 2 sec
      autoplayHoverPause: false,
      smartSpeed: 600, // smooth speed
      responsive: {
        0: { items: 1 },
        768: { items: 3 } // show 3 on medium/desktop
      }
    });
  });
...
function initHome66() {
  ...
  sections.forEach((section, i) => {
    ScrollTrigger.create({
      ...
      pin: true,
      pinSpacing: i === sections.length - 1, // only last section keeps spacing
      scrub: true,
      markers: false
    });
  });
